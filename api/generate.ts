...
const productSchema = {
  ...
  properties: {
    ...
    fullDescription: {
      type: Type.STRING,
      description: "توضیحات کامل محصول با فرمت HTML. این توضیحات باید با یک پاراگراف مقدمه جذاب شروع شود که شامل نام محصول به صورت **bold** است. بخش‌های مختلف باید با تیترهای بولد شده (`<strong>`) از هم جدا شوند.",
    },
    shortDescription: {
        type: Type.STRING,
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **قوانین نگارشی برای سئو (بسیار مهم):**
        *   **استفاده از تیتر:** برای جدا کردن هر بخش از متن، از یک تیتر **بولد شده** با استفاده از تگ \`<strong>\` استفاده کن. **از ایموجی یا تگ‌های h1, h2, h3 و غیره استفاده نکن.** این یک الزام است.
    ...
    *   **ساختار الزامی و دقیق متن:**
        ...
        *   **<strong>سفری به خاستگاه</strong> (حدود ۸۰ کلمه):** به طور مفصل در مورد منطقه کشت و اصالت محصول بنویس. ویژگی‌های جغرافیایی یا فرهنگی که این محصول را خاص می‌کند را توصیف کن.
        *   **<strong>تجربه طعم و عطر</strong> (حدود ۷۰ کلمه):** طعم، بافت، و عطر محصول را با جزئیات دقیق و وسوسه‌انگیز توصیف کن. از صفات توصیفی غنی استفاده کن.
        *   **<strong>گنجینه‌ای از خواص</strong> (حدود ۱۰۰ کلمه):** خواص و فواید محصول برای سلامتی را به تفصیل شرح بده. به ویتامین‌ها، مواد معدنی، و تاثیرات مثبت آن بر بدن اشاره کن. این بخش باید بسیار کامل باشد.
        *   **<strong>همراه لحظه‌های شما</strong> (حدود ۵۰ کلمه):** کاربردهای مختلف محصول را بیان کن. از مصرف به عنوان میان‌وعده تا استفاده در آشپزی، شیرینی‌پزی یا همراهی با نوشیدنی‌های مختلف.
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **قوانین نگارشی برای سئو (بسیار مهم):**
        *   **استفاده از تیتر:** برای جدا کردن هر بخش از متن، از یک تیتر **بولد شده** با استفاده از تگ \`<strong>\` استفاده کن. **از ایموجی یا تگ‌های h1, h2, h3 و غیره استفاده نکن.** این یک الزام است.
        *   **کوتاهی جملات:** جملات را کوتاه و روان بنویس. **اکثر جملات باید زیر ۲۵ کلمه باشند.**
        *   **لحن فعال (Active Voice):** همیشه از لحن فعال استفاده کن و از حالت مجهول (passive voice) پرهیز کن.
    *   **ساختار الزامی و دقیق:**
        *   **پاراگراف مقدمه:** با یک پاراگراف کوتاه (۲-۳ جمله)، جذاب و شامل نام محصول فارسی به صورت **bold** (\`<strong>...\`) شروع شود. این بخش تیتر ندارد.
        *   **بخش‌ها:** هر بخش باید با یک تیتر بولد (\`<strong>\`) شروع شود و محتوای آن مختصر باشد (نهایتاً ۲–۳ خط یا ۲–۴ بولت در یک \`<ul>\`).
          *   تیتر \`<strong>ویژگی‌های اصلی</strong>\` و یک لیست بالت (\`<ul>\`) از ویژگی‌ها.
          *   تیتر \`<strong>مزایای استفاده</strong>\` و یک لیست بالت از مزایا.
          *   تیتر \`<strong>طریقه مصرف</strong>\` (فقط اگر مرتبط است).
          *   تیتر \`<strong>مناسب چه کسانی‌ست؟</strong>\`.
          *   تیتر \`<strong>مشخصات محصول</strong>\` (شامل برند، وزن، بسته‌بندی، کشور سازنده و غیره).
          *   تیتر \`<strong>نکات مهم</strong>\`.`;
