...
const productSchema = {
  ...
  properties: {
    ...
    fullDescription: {
      type: Type.STRING,
      description: "توضیحات کامل محصول با فرمت HTML دقیقاً مطابق ساختار درخواستی در پرامپت. این توضیحات باید با نام فارسی و انگلیسی محصول شروع شود و سپس شامل مقدمه، و بخش‌های دیگر باشد که هر بخش با یک تگ <hr> از دیگری جدا شده است.",
    },
    shortDescription: {
      type: Type.STRING,
      description: "یک توضیح کوتاه (۲-۳ خط) و جذاب برای محصول، مناسب برای نمایش در لیست محصولات یا زیر عنوان اصلی.",
    },
    seoTitle: {
      type: Type.STRING,
  ...
  required: ["correctedProductName", "englishProductName", "fullDescription", "shortDescription", "seoTitle", "slug", "focusKeyword", "metaDescription", "keyphraseSynonyms"],
};
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **ساختار الزامی و دقیق:**
        *   **ردیف نام محصول:** با نام فارسی و انگلیسی محصول که هر دو **بولد** شده‌اند شروع شود. مانند: \`<p><strong>نام فارسی کامل محصول</strong></p><p><strong>نام انگلیسی کامل محصول</strong></p>\`
        *   **پاراگراف مقدمه:** یک پاراگراف کوتاه (۲-۳ جمله)، جذاب و شامل کلیدواژه کانونی.
        *   **بخش "ویژگی‌های اصلی":** با عنوان \`<strong>✅ ویژگی‌های اصلی:\`</strong> و یک لیست بالت (\`<ul>\`) از ۴ تا ۶ ویژگی.
        *   **بخش "مزایای استفاده":** با عنوان \`<strong>✨ مزایای استفاده:\`</strong> و یک لیست بالت از ۳ تا ۴ مزیت.
        *   **بخش "طریقه مصرف":** با عنوان \`<strong>📌 طریقه مصرف:\`</strong> و ۲-۳ بولت یا ۲ خط توضیح.
        *   **بخش "مناسب چه کسانی‌ست؟":** با عنوان \`<strong>🔍 مناسب چه کسانی‌ست؟:\`</strong> و ۲-۳ خط توضیح یا بولت.
        *   **بخش "مشخصات محصول":** با عنوان \`<strong>📦 مشخصات محصول:\`</strong> و لیستی از مشخصات کلیدی.
        *   **بخش "نکات مهم":** با عنوان \`<strong>🟢 نکات مهم:\`</strong> و یک لیست بالت از ۲ تا ۳ نکته.

2.  **توضیحات کوتاه (\`shortDescription\`):**
    *   یک پاراگراف کوتاه و بسیار جذاب برای نمایش زیر عنوان محصول.
...
7.  **تجزیه و تحلیل سئو برتر (\`keyphraseSynonyms\`):**
    *   آرایه‌ای از ۳ تا ۵ عبارت مرتبط.
