...
const productSchema = {
  ...
  properties: {
    ...
    fullDescription: {
      type: Type.STRING,
      description: "توضیحات کامل، جامع و حرفه‌ای محصول با فرمت HTML دقیقاً مطابق ساختار درخواستی در پرامپت. این توضیحات باید جزئیات کاملی مانند ویژگی‌ها، مشخصات فنی (وزن، ابعاد، مواد تشکیل‌دهنده و...) و کاربردها را پوشش دهد تا به سوالات خریدار پاسخ دهد.",
    },
    shortDescription: {
        type: Type.STRING,
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **ساختار الزامی:**
        ...
        *   **لیست مشخصات:** یک لیست جامع و دقیق از مشخصات فنی و فیزیکی محصول. تا جای ممکن تمام جزئیات مربوط به محصول مانند ابعاد، وزن، جنس، کشور سازنده، تاریخ انقضا، مواد تشکیل‌دهنده و غیره را ذکر کنید. هر مشخصه را با \`<strong>\` شروع کنید (مانند \`<strong>وزن:</strong> 100 گرم\`). هر مشخصه در یک خط جدید باشد (از \`<br>\` استفاده کنید).
