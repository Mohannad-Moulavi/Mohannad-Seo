...
const productSchema = {
  ...
  properties: {
    ...
    seoTitle: {
      type: Type.STRING,
      description: "عنوان سئو جذاب و بهینه (بین ۵۵ تا ۶۰ کاراکتر) شامل کلیدواژه کانونی و کلمات کلیدی مانند 'خرید' یا 'قیمت'."
    },
    slug: {
      type: Type.STRING,
    ...
    altImageText: {
        type: Type.STRING,
        description: "متن جایگزین (alt text) توصیفی و بهینه برای تصویر محصول (بین ۸ تا ۱۲ کلمه) که شامل کلیدواژه کانونی باشد."
    },
...
- **طول متن:** کل توضیحات باید بین ۳۰۰ تا ۴۰۰ کلمه باشد. هر پاراگراف باید بین ۴۰ تا ۷۰ کلمه (معادل ۲ تا ۴ جمله) باشد و هر جمله حداکثر ۲۰ کلمه باشد.
- **خوانایی:** حدود ۲۵ تا ۳۰ درصد جملات باید از کلمات انتقالی (مثل: همچنین، بنابراین، علاوه بر این، به همین دلیل) استفاده کنند. استفاده از صدای مجهول باید کمتر از ۱۰٪ باشد.
- **استفاده از کلیدواژه کانونی:** کلیدواژه باید در پاراگراف اول (۵۰ کلمه ابتدایی) بیاید و به طور طبیعی ۳ تا ۵ بار در کل متن تکرار شود.
...
# 2. ساختار و فرمت متن
- **پاراگراف مقدمه:** با یک پاراگراف جذاب و کامل (حدود ۴۰-۷۰ کلمه) شروع کن که محصول را معرفی می‌کند و شامل نام محصول به صورت \`<strong>\` است.
- **بخش‌های تطبیقی (Adaptive Sections):** ساختار بخش‌ها باید **بر اساس نوع محصول** هوشمندانه انتخاب شود. هر بخش باید با یک تیتر **bold** شده همراه با یک ایموجی مناسب شروع شود (مثال: \`<p><strong>📦 ویژگی‌های اصلی:</strong></p>\`). از تگ‌های هدر (h1-h6) استفاده نکن.
    - **مثال‌های دقیق برای الهام گرفتن:**
        - **لوازم الکترونیکی:** ویژگی‌ها، مشخصات فنی، راهنمای استفاده، گارانتی، مزایا.
        - **اسباب‌بازی:** رده‌بندی سنی، نکات ایمنی، ارزش آموزشی، جنس و نحوه نگهداری.
        - **پوشاک و اکسسوری:** جنس، راهنمای سایز، نکات ست کردن، دستورالعمل نگهداری.
        - **لوازم خانه و مبلمان:** جنس، نکات نگهداری، ویژگی‌های منحصربه‌فرد، مزایا.
        - **لوازم آرایشی:** ترکیبات، دستورالعمل استفاده، مزایا، هشدارها.
        - برای سایر دسته‌بندی‌ها، بخش‌های منطقی و مرتبط را به صورت پویا انتخاب کن و از ایجاد بخش‌های نامرتبط خودداری کن.
- **جداکننده:** بعد از هر بخش (به جز آخرین بخش)، از یک جداکننده متنی \`───\` در یک پاراگراف جداگانه استفاده کن (\`<p>───</p>\`). **به هیچ وجه از تگ \`<hr />\` استفاده نکن.**
- **لیست‌ها:** برای لیست کردن ویژگی‌ها یا مشخصات، از تگ \`<ul>\` و \`<li>\` استفاده کن.
- **لحن:** لحن متن باید دوستانه، حرفه‌ای و متقاعدکننده باشد.

# 3. مشخصات محصول
- در یکی از بخش‌ها، حتماً اطلاعات ساختاریافته مانند برند، نوع، سایز/وزن، رنگ و نوع بسته‌بندی را ذکر کن.
`;
