...
const productSchema = {
  ...
  properties: {
    ...
    fullDescription: {
      type: Type.STRING,
      description: "توضیحات کامل و کوتاه محصول با فرمت HTML دقیقاً مطابق ساختار درخواستی در پرامپت. این توضیحات باید شامل مقدمه (با نام بولد شده محصول)، ویژگی‌های کلیدی، پیشنهاد مصرف/روش استفاده، روش نگهداری، مشخصات و نکات مهم باشد که هر بخش با یک تگ <hr> از دیگری جدا شده است.",
    },
    focusKeyword: {
      type: Type.STRING,
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **ساختار الزامی و دقیق:**
        ...
        *   **بخش "پیشنهاد مصرف / روش استفاده" (اختیاری):**
            *   فقط اگر برای محصول کاملاً مرتبط است، این بخش را اضافه کنید. در غیر این صورت، این بخش را کاملاً حذف کنید.
            *   بر اساس نوع محصول، **عنوان مناسب را انتخاب کنید**: "پیشنهاد مصرف" برای خوراکی‌ها، "روش استفاده" برای محصولات غیرخوراکی و بهداشتی، و غیره.
            *   یک **ایموجی مرتبط با عنوان** انتخاب کنید (مثلاً 🍽 برای غذا، 💡 برای راهنمایی، 🧼 برای محصولات بهداشتی).
            *   عنوان را به این صورت فرمت کنید: \`<strong>[ایموجی] [عنوان انتخابی]:\`</strong> (مثلاً \`<strong>🍽 پیشنهاد مصرف:\`</strong> یا \`<strong>💡 روش استفاده:\`</strong>).
            *   محتوای این بخش را در قالب یک لیست بالت (\`<ul>\`) از ۲ تا ۴ مورد بنویسید.
