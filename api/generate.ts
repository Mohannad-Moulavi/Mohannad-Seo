...
const productSchema = {
  ...
  properties: {
    ...
    metaDescription: {
        type: Type.STRING,
        description: "توضیحات متا جذاب برای گوگل (بین ۱۲۰ تا ۱۵۰ کاراکتر) که شامل کلیدواژه کانونی، یک مزیت کلیدی و یک فراخوان به اقدام (CTA) باشد."
    },
    altImageText: {
        type: Type.STRING,
...
- **طول متن:** کل توضیحات باید بین ۲۵۰ تا ۳۵۰ کلمه باشد. هر پاراگراف باید بین ۴۰ تا ۶۰ کلمه (حداکثر ۴ جمله) باشد.
- **خوانایی:** از جملات کوتاه و کلمات انتقالی برای بهبود خوانایی استفاده کن.
...
# 2. ساختار و فرمت متن
- **پاراگراف مقدمه:** با یک پاراگراف جذاب (حدود ۴۰-۶۰ کلمه) شروع کن که محصول را معرفی می‌کند و شامل نام محصول به صورت \`<strong>\` است.
- **بخش‌های تطبیقی (Dynamic Sections):** ساختار بخش‌ها باید **بر اساس نوع محصول** هوشمندانه انتخاب شود. هر بخش باید با یک تیتر **bold** شده همراه با یک ایموجی مناسب شروع شود (مثال: \`<p><strong>📦 ویژگی‌ها:</strong></p>\`). از تگ‌های هدر (h1-h6) استفاده نکن. **بخش‌های نامرتبط را به صورت خودکار حذف کن.**
    - **مثال‌های دقیق برای الهام گرفتن:**
        - **غذا و نوشیدنی:** پیشنهاد مصرف | ترکیبات | روش نگهداری
        - **لوازم الکترونیکی:** مشخصات فنی | ویژگی‌ها | گارانتی
        - **اسباب‌بازی:** رده سنی | نکات ایمنی | ارزش آموزشی
        - **پوشاک و اکسسوری:** جنس و نگهداری | راهنمای سایز | نکات استایل
        - **لوازم آرایشی:** ترکیبات | روش مصرف | هشدارها
...
# 3. مثال فرمت کلی:
<p><strong>✨ توضیحات محصول</strong></p>
<p>متن مقدمه (۴۰ تا ۶۰ کلمه)...</p>
<p>───</p>
...
<p>───</p>
<p><strong>☕ پیشنهاد مصرف</strong></p>
<p>متن پاراگراف (۴۰ تا ۶۰ کلمه)...</p>
`;
