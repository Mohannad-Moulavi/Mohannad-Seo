...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **ساختار الزامی و دقیق:**
        ...
        *   **بدنه اصلی:** در ادامه، با استفاده از پاراگراف‌ها و تیترهای \`<h4>\`, محصول را به طور کامل معرفی کن. ویژگی‌ها، مزایا، و کاربردهای آن را شرح بده.
        *   **الزامات تیترها:** هر تیتر \`<h4>\` باید با یک ایموجی (استیکر) مرتبط شروع شود. مثال: \`<h4>✨ ویژگی‌های کلیدی</h4>\`
        *   **الزامات جداکننده:** بین هر بخش که با \`<h4>\` شروع می‌شود و بخش بعدی، یک خط جداکننده افقی (\`<hr />\`) قرار بده.
        *   در صورت لزوم از لیست بالت (\`<ul>\`) استفاده کن.
