...
1.  **پاراگراف مقدمه:** یک پاراگراف جذاب و کامل در معرفی محصول (حدود ۲ تا ۳ جمله).
2.  **بخش‌ها:** **دقیقاً** شامل بخش‌های زیر باشد که هر کدام با یک تیتر \`h5\` شروع می‌شود. **حتماً و حتماً در ابتدای هر تیتر \`h5\` ایموجی مشخص شده را قرار بده.**
    -   \`<h5>✅ ویژگی‌های اصلی:</h5>\` (لیستی از ویژگی‌های کلیدی)
    -   \`<h5>✨ مزایای استفاده:</h5>\` (لیستی از مزایای اصلی)
    -   \`<h5>📌 طریقه مصرف:</h5>\` (یک پاراگراف کوتاه و واضح)
    -   \`<h5>🔍 مناسب چه کسانی‌ست؟:</h5>\` (یک پاراگراف کوتاه که مخاطب هدف را مشخص می‌کند)
    -   \`<h5>📦 مشخصات محصول:</h5>\` (لیستی از مشخصات فنی/فیزیکی)
    -   \`<h5>🟢 نکات مهم:</h5>\` (لیستی از نکات تکمیلی یا توصیه‌ها)
...
5.  **قواعد Yoast SEO:** تمام متن باید طبق اصول Yoast SEO نوشته شود (استفاده از کلیدواژه، خوانایی بالا، پاراگراف‌های کوتاه).
6.  **لحن:** لحن متن باید دوستانه، حرفه‌ای و متقاعدکننده باشد، با جملات کوتاه و روان.
`;
