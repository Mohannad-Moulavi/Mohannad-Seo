...
const productSchema = {
  ...
  properties: {
    ...
    fullDescription: {
      type: Type.STRING,
      description: "توضیحات کامل محصول با فرمت HTML. این توضیحات باید با یک پاراگراف مقدمه جذاب شروع شود که شامل نام محصول به صورت **bold** است. بخش‌های مختلف باید با تیترهای `<h2>` از هم جدا شوند.",
    },
    shortDescription: {
        type: Type.STRING,
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **قوانین نگارشی برای سئو (بسیار مهم):**
        *   **استفاده از تیتر:** برای جدا کردن هر بخش از متن، از یک تیتر با تگ \`<h2>\` استفاده کن. **فقط از تگ h2 استفاده کن و از هیچ تگ دیگری مثل h1, h3 و غیره، یا ایموجی، یا تگ strong استفاده نکن.** این یک الزام است.
    ...
    *   **ساختار الزامی و دقیق متن:**
        ...
        *   **<h2>سفری به خاستگاه</h2> (حدود ۸۰ کلمه):** به طور مفصل در مورد منطقه کشت و اصالت محصول بنویس. ویژگی‌های جغرافیایی یا فرهنگی که این محصول را خاص می‌کند را توصیف کن.
        *   **<h2>تجربه طعم و عطر</h2> (حدود ۷۰ کلمه):** طعم، بافت، و عطر محصول را با جزئیات دقیق و وسوسه‌انگیز توصیف کن. از صفات توصیفی غنی استفاده کن.
        *   **<h2>گنجینه‌ای از خواص</h2> (حدود ۱۰۰ کلمه):** خواص و فواید محصول برای سلامتی را به تفصیل شرح بده. به ویتامین‌ها، مواد معدنی، و تاثیرات مثبت آن بر بدن اشاره کن. این بخش باید بسیار کامل باشد.
        *   **<h2>همراه لحظه‌های شما</h2> (حدود ۵۰ کلمه):** کاربردهای مختلف محصول را بیان کن. از مصرف به عنوان میان‌وعده تا استفاده در آشپزی، شیرینی‌پزی یا همراهی با نوشیدنی‌های مختلف.
...
1.  **توضیحات کامل محصول (\`fullDescription\`):**
    ...
    *   **ساختار الزامی و دقیق:**
        ...
        *   **بخش‌ها:** هر بخش باید با یک تیتر \`<h2>\` شروع شود و محتوای آن مختصر باشد (نهایتاً ۲–۳ خط یا ۲–۴ بولت در یک \`<ul>\`).
          *   تیتر \`<h2>ویژگی‌های اصلی</h2>\` و یک لیست بالت (\`<ul>\`) از ویژگی‌ها.
          *   تیتر \`<h2>مزایای استفاده</h2>\` و یک لیست بالت از مزایا.
          *   تیتر \`<h2>طریقه مصرف</h2>\` (فقط اگر مرتبط است).
          *   تیتر \`<h2>مناسب چه کسانی‌ست؟</h2>\`.
          *   تیتر \`<h2>مشخصات محصول</h2>\` (شامل برند، وزن، بسته‌بندی، کشور سازنده و غیره).
          *   تیتر \`<h2>نکات مهم</h2>\`.`;
